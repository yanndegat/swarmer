...
The private insecure registry which is automatically started on the swarm cluster is registered on the "regsitry.service.consul" name. So you have to tag & push docker images with this name if you want the nodes to be able to download your images. To do so, your box has to resolv the correct host on which the registry is running. To help you push images to this registry, a docker-compose file is provided ([[file:/proxy-registry-compose.yml]]).

This compose file starts a consul agent and a dnsmasq service with net=host. The agent is configured to join the consul cluster.
...
root $ exit
$ docker-compose -d -f proxy-registry-compose.yml up
$ curl registry.service.consul:5000/v2/_catalog
{"repositories":[]}
$ ...
#+END_SRC

You can now push images to this registry.
