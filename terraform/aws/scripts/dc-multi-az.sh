...
TF_CONSUL_VARFILE=/tmp/terraform.consul.$$.tfvars
...
bootstrap(){
    ...
    $TFCMD -c docker-registry -i priv -f $TF_VARFILE apply
    REGISTRY_ADDRESS=$($TFCMD -c docker-registry -i priv output | grep "^registry_address =" | cut -d= -f2 | sed 's/"//g')

    cat >> $TF_VARFILE <<EOF
additional_docker_opts = "--insecure-registry=$REGISTRY_ADDRESS"
EOF
